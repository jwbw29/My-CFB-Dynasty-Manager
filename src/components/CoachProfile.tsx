...
const CoachProfile = () => {
  ...
  useEffect(() => {
    ...
    const loadData = () => {
      try {
        ...
        console.error("Error loading profile/year:", error);
      }
    };
    loadData();
    const handleStorageChange = (event: StorageEvent) => {
      if (event.key === "currentYear" || event.key === "dynastyLoaded")
        loadData();
    };
  ...
  const ColorPreview = ({
    ...
    <div
      className={`${size} rounded border-2 border-gray-300 dark:border-gray-600`}
  ...
  return (
    <>
      <Button
        variant="ghost"
        className="flex flex-wrap items-center justify-center sm:justify-start gap-x-4 gap-y-1 px-2 h-auto"
        onClick={() => setIsEditing(true)}
        style={{
          borderLeft: profile.schoolColors?.primary
            ? `4px solid ${profile.schoolColors.primary}`
            : undefined,
        }}
      >
        ...
        {profile.schoolColors && (
          <div className="flex items-center space-x-1">
            <ColorPreview color={profile.schoolColors.primary} size="w-4 h-4" />
            <ColorPreview
              color={profile.schoolColors.secondary}
              size="w-4 h-4"
            />
          </div>
        )}
        ...
        <DialogContent className="sm:max-w-[600px]">
          ...
          <div className="grid gap-6 py-4">
            <div className="grid grid-cols-4 items-center gap-4">
              ...
              <Input
                ...
                onChange={(e) =>
                  setProfile((p) => ({ ...p, coachName: e.target.value }))
                }
            ...
            <div className="grid grid-cols-4 items-center gap-4">
              ...
              <Select
                ...
                value={
                  profile.conference ||
                  getTeamByName(profile.schoolName)?.conference ||
                  ""
                }
                onValueChange={(value) =>
                  setProfile((p) => ({ ...p, conference: value }))
                }
            ...
            <div className="grid grid-cols-4 items-center gap-4">
              ...
              <Input
                ...
                value={displayYear}
                onChange={(e) =>
                  setDisplayYear(parseInt(e.target.value) || defaultYear)
                }
                className="col-span-3"
              />
            </div>
                ...
                <div className="space-y-2">
                  ...
                  <div className="flex items-center gap-2">
                    <input
                      id="primaryColor"
                      type="color"
                      value={profile.schoolColors?.primary}
                      onChange={(e) =>
                ...
                <div className="space-y-2">
                  ...
                  <div className="flex items-center gap-2">
                    <input
                      id="secondaryColor"
                      type="color"
                      value={profile.schoolColors?.secondary}
                      onChange={(e) =>
                ...
                <div className="space-y-2">
                  ...
                  <div className="flex items-center gap-2">
                    <input
                      id="accentColor"
                      type="color"
                      value={profile.schoolColors?.accent}
                      onChange={(e) =>
          ...
          <DialogFooter>
            ...
            <Button
              onClick={handleSave}
              style={{
                backgroundColor: profile.schoolColors?.primary,
                color: "white",
              }}
            >
              Save Profile
