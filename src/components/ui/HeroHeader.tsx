...
export const HeroHeader: React.FC<HeroHeaderProps> = ({
  ...
  return (
    <div
      className={`relative overflow-hidden rounded-2xl bg-primary ${className}`}
    >
      ...
      <div className="relative p-6 md:p-8">
        ...
        <div className="flex flex-col justify-center items-center text-center gap-2">
          ...
          <h1 className="text-4xl md:text-5xl leading-relaxed md:leading-relaxed font-black text-school-secondary flex items-center gap-3 justify-center">
            {Icon && <Icon className="h-10 w-10 text-school-secondary" />}
            {title}
          </h1>

          {/* Optional subtitle */}
          {subtitle && (
            <p className="text-lg font-semibold text-school-accent">
              {subtitle}
            </p>
          )}
