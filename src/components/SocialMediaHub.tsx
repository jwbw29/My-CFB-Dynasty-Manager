...
const AvatarFallback: React.FC<{
  ...
  <div
    className={`w-full h-full flex items-center justify-center text-white font-medium text-sm ${className}`}
  >
    {children}
  </div>
);
...
const MEDIA_PERSONALITIES = {
  official: (teamName: string) => ({
    name: `${teamName} Football`,
    handle: `@${teamName.replace(/\s+/g, "")}FB`,
    avatar: undefined,
    verified: true,
  }),
  on3: {
    name: "On3 Recruits",
    handle: "@On3Recruits",
    avatar: "/avatars/on3.png",
    verified: true,
  },
  espn: {
    name: "ESPN College Football",
    handle: "@ESPNCFB",
    avatar: "/avatars/espncfb.png",
    verified: true,
  },
  br: {
    name: "Bleacher Report CFB",
    handle: "@BR_CFB",
    avatar: "/avatars/brcfb.png",
    verified: true,
  },
  pff: {
    name: "PFF College",
    handle: "@PFF_College",
    avatar: "/avatars/pff.png",
    verified: true,
  },
  gameday: {
    name: "College GameDay",
    handle: "@CollegeGameDay",
    avatar: "/avatars/gameday.png",
    verified: true,
  },
  ncaa: {
    name: "NCAA Football",
    handle: "@NCAAFootball",
    avatar: "/avatars/ncaafootball.png",
    verified: true,
  },
  playoff: {
    name: "CFB Playoffs",
    handle: "@CFBPlayoff",
    avatar: "/avatars/cfbplayoff.png",
    verified: true,
  },
  reporter: {
    name: "CFB Report",
    handle: "@CFBReport",
    avatar: "/avatars/cfbrep.png",
    verified: true,
  },
  recruiting247: {
    name: "24/7 Recruiting",
    handle: "@247Recruiting",
    avatar: "/avatars/247.png",
    verified: true,
  },
};

const generateGameRecapPosts = (
  game: Game,
  teamName: string,
  players: Player[],
  virtualDate: Date
): SocialPost[] => {
  if (!game || !game.opponent || game.result === "N/A" || game.result === "Bye")
    return [];

  const isWin = game.result === "Win";
  const scoreParts = game.score.split("-").map((s) => parseInt(s.trim()));
  const scoreDiff =
    scoreParts.length === 2 ? Math.abs(scoreParts[0] - scoreParts[1]) : 0;
  const topPerformer =
    players.length > 0
      ? players.sort((a, b) => parseInt(b.rating) - parseInt(a.rating))[0]
      : null;

  const templates = isWin
    ? [
        `That's a W! Final from today's game: ${teamName} ${game.score} ${
          game.opponent
        }. Great team effort! #Go${teamName.replace(/\s+/g, "")}`,
        scoreDiff > 21
  ...
  const post: SocialPost = {
    id: `official-recap-${game.id}`,
    type: "game_recap",
    author: official.name,
    authorHandle: official.handle,
    authorType: "official",
    content: getRandomTemplate(templates),
    timestamp: new Date(virtualDate.getTime() - 18 * 3600000),
    likes: isWin ? 450 : 150,
    retweets: isWin ? 120 : 40,
    comments: 60,
    verified: true,
  };

  const mediaPersonalitiesArray = Object.values(MEDIA_PERSONALITIES).filter(
    (
      p
    ): p is {
      name: string;
      ...
      typeof p !== "function" &&
      p.handle !== "@On3Recruits" &&
      p.handle !== "@247Recruiting"
  );
  const mediaPersonality =
    mediaPersonalitiesArray[
      Math.floor(Math.random() * mediaPersonalitiesArray.length)
    ];
  ...
  const mediaTemplates = isWin
    ...
    : [
        `UPSET ALERT? ${game.opponent} takes down ${teamName} in a ${
          scoreDiff <= 7 ? "thriller" : "convincing fashion"
        }, ${game.score}.`,
  ...
  const mediaPost: SocialPost = {
    id: `media-recap-${game.id}`,
    type: "news",
    author: mediaPersonality.name,
    authorHandle: mediaPersonality.handle,
    authorType: "media",
    content: getRandomTemplate(mediaTemplates),
    timestamp: new Date(virtualDate.getTime() - 16 * 3600000),
    likes: 300,
    retweets: 90,
    comments: 75,
    verified: mediaPersonality.verified,
    avatarPath: mediaPersonality.avatar,
  };

  return [post, mediaPost];
};

const generatePlayerSpotlightPost = (
  players: Player[],
  teamName: string,
  virtualDate: Date
): SocialPost[] => {
  ...
  const positionPraise: { [key: string]: string } = {
    QB: "A true field general and leader of this offense.",
    RB: "A workhorse out of the backfield. Tough to bring down.",
    WR: "A nightmare for defensive backs. Can score from anywhere.",
    TE: "The ultimate safety blanket and a red zone threat.",
    DL: "Lives in the opponent's backfield. A true trench warrior.",
    LB: "The heart and soul of the defense. Sideline to sideline speed.",
    CB: "Welcome to Revis Island. Good luck throwing his way.",
    S: "The commander of the secondary. Lays the boom.",
    OL: "The unsung hero paving the way for our offense.",
    K: "Automatic. Mr. Reliable when we need points.",
    P: "A weapon in the field position battle.",
  };

  const official = MEDIA_PERSONALITIES.official(teamName);
  return [
    {
      id: `player-spotlight-${topPlayer.id}`,
      ...
      timestamp: new Date(virtualDate.getTime() - 2 * 86400000),
      likes: 300,
      retweets: 80,
      comments: 50,
      verified: true,
...
const generateRecruitingPosts = (
  recruits: Recruit[],
  teamName: string,
  virtualDate: Date
): SocialPost[] => {
  if (recruits.length === 0) return [];
  const topRecruit = recruits.sort(
  ...
  return [
    {
      id: `recruit-buzz-${topRecruit.id}`,
      type: "recruiting",
      author: media.name,
      authorHandle: media.handle,
      authorType: "media",
      content,
      timestamp: new Date(virtualDate.getTime() - 3 * 86400000),
      likes: 600,
      retweets: 250,
      comments: 120,
      verified: true,
...
const generateFanPost = (
  ...
  if (lastGame?.result === "Win") {
    templates = [
      `LET'S GOOOO! What a win! This team is DIFFERENT this year! #Go${teamName.replace(
        /\s+/g,
        ""
      )}`,
      `So proud of this team. They left it all out on the field today. Huge W!`,
    ];
  } else if (lastGame?.result === "Loss") {
    templates = [
      `Tough loss, but we'll bounce back. Gotta clean up those mistakes for next week. #Believe`,
      `Frustrating game. We had our chances. On to the next, I still ride with this team.`,
    ];
  } else {
    templates = [
      `Can't wait for the season to start! I think this is our year. #Go${teamName.replace(
        /\s+/g,
        ""
      )}`,
    ];
  ...
  return [
    {
      id: `fan-post-${Date.now()}`,
      type: "tweet",
      author: "Big CFB Fan",
      authorHandle: "@CFBFanatic99",
      authorType: "fan",
      content: getRandomTemplate(templates),
      timestamp: new Date(virtualDate.getTime() - 10 * 3600000),
...
const SocialMediaHub: React.FC = () => {
  ...
  const dynastyData = useMemo(() => {
    ...
    return {
      ...
      recruits: getAllRecruits().filter(
        (r) => r.recruitedYear === rawCurrentYear
      ),
      players: getPlayers(),
      year: rawCurrentYear,
    };
  ...
  const generateAllPosts = useCallback(() => {
    if (!dynastyData || dynastyData.teamName === "Your Team" || isGenerating)
      return;
    ...
    allGeneratedPosts.push(
      ...generatePlayerSpotlightPost(players, teamName, virtualToday)
    );

    if (recruits.length > 0) {
      allGeneratedPosts.push(
        ...generateRecruitingPosts(recruits, teamName, virtualToday)
      );
    }

    allGeneratedPosts.push(
      ...generateFanPost(teamName, lastCompletedGame, virtualToday)
    );
    ...
    if (allGeneratedPosts.length === 0) {
      ...
      allGeneratedPosts.push({
        ...
        authorHandle: official.handle,
        authorType: "official",
        content: `Let's get this season started! The journey for a championship begins now. #Go${teamName.replace(
          /\s+/g,
          ""
        )} #CFB${year - 2000 + 25}`,
        timestamp: virtualToday,
        likes: 50,
        retweets: 10,
        comments: 5,
        verified: true,
  ...
  const filteredPosts = useMemo(() => {
    ...
    return posts.filter((post) => {
      ...
      if (filter === "social")
        return ["player_tweet", "player_spotlight"].includes(post.type);
  ...
  const getAuthorAvatarColor = (authorType: string) =>
    ({
      ...
      media: "bg-purple-600",
      player: "bg-orange-600",
      fan: "bg-gray-600",
    }[authorType] || "bg-gray-600");

  const getTypeIcon = (type: string) =>
    ({
      game_recap: <Trophy className="h-4 w-4" />,
      recruiting: <Star className="h-4 w-4" />,
      news: <TrendingUp className="h-4 w-4" />,
      player_spotlight: <Users className="h-4 w-4" />,
      ...
      <div className="space-y-4">
        ...
        {!isGenerating && filteredPosts.length === 0 && (
          <Card className="p-8 text-center text-muted-foreground">
            ...
            No posts yet. Play games to see the feed come to life!
          </Card>
        )}
        {filteredPosts.map((post) => (
          <Card key={post.id} className="hover:shadow-lg transition-shadow">
            <CardContent className="p-4 sm:p-6">
                    ...
                    <img
                      ...
                      onError={(e) => {
                        ...
                        (target.parentElement as HTMLElement).classList.add(
                          getAuthorAvatarColor(post.authorType)
                        );
                      }}
                    />
                  ) : (
                    <AvatarFallback
                      className={getAuthorAvatarColor(post.authorType)}
                    >
                      {post.author
                        .split(" ")
                        .map((n) => n[0])
              ...
              <div className="mb-4">
                <p className="whitespace-pre-wrap text-sm leading-relaxed md:leading-relaxed">
                  {post.content}
                </p>
              </div>
