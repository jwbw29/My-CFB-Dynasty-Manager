...
const TeamOverviewCard: React.FC<TeamOverviewCardProps> = ({
  players,
  teamName,
}) => {
  ...
  const offensePlayers = players.filter((p) =>
    offensePositions.includes(p.position)
  );
  const defensePlayers = players.filter((p) =>
    defensivePositions.includes(p.position)
  );
  const specialTeamsPlayers = players.filter((p) =>
  ...
  const calculateOverall = (positionPlayers: Player[]) => {
    ...
    const total = positionPlayers.reduce(
      (sum, player) => sum + (parseInt(player.rating) || 0),
      0
    );
