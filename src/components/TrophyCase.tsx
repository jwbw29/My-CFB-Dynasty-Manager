...
const TrophyCase: React.FC = () => {
  ...
  const migrateTrophies = (trophies: Trophy[]) => {
    return trophies.map((trophy) => {
      ...
      let newCategory: Trophy["category"] =
        trophy.category as Trophy["category"];
      ...
      if (typeof trophy.category === "string") {
        ...
        // Default fallback - but preserve original if it's already valid
          ...
          !TROPHY_CATEGORIES[trophy.category as keyof typeof TROPHY_CATEGORIES]
        ) {
          newCategory = "championship";
        }
      }
  ...
  React.useEffect(() => {
    ...
    const hasChanged =
      JSON.stringify(migratedTrophies) !== JSON.stringify(allTrophies);
  ...
  const getCategoryInfo = (category: string) => {
    return (
      TROPHY_CATEGORIES[category as keyof typeof TROPHY_CATEGORIES] ||
      TROPHY_CATEGORIES.championship
    );
  };
  ...
  const getTrophyIcon = (category: string, size: string = "h-8 w-8") => {
    const IconComponent =
      TROPHY_CATEGORIES[category as keyof typeof TROPHY_CATEGORIES]?.icon ||
      Trophy;
    ...
    <div className="space-y-8 max-w-7xl mx-auto">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-5 gap-4">
        ...
        {Object.entries(TROPHY_CATEGORIES).map(([key, category]) => (
          <Card
            key={key}
            className={`bg-gradient-to-br ${category.bgColor} ${category.borderColor}`}
          >
            <CardContent className="pt-6">
              <div className="text-center">
                {getTrophyIcon(key)}
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mt-4">
              ...
              <div className="space-y-2">
                ...
                <Input
                  ...
                  onChange={(e) =>
                    setNewTrophy({ ...newTrophy, description: e.target.value })
                  }
              ...
              <div className="space-y-2">
                ...
                <Input
                  ...
                  onChange={(e) =>
                    setNewTrophy({ ...newTrophy, location: e.target.value })
                  }
      ...
      >
        ...
        <TabsContent value="all" className="space-y-6">
          ...
          <ScrollArea className="h-[600px] p-1">
            {" "}
            {/* MODIFICATION: Added padding for scrollbar */}
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {getFilteredTrophies().map((trophy) => {
                ...
                return (
                  <Card
                    key={trophy.id}
                    className={`hover:shadow-lg transition-shadow ${categoryInfo.bgColor} ${categoryInfo.borderColor}`}
                  >
                    <CardContent className="pt-6">
                      <div className="flex items-start justify-between mb-4">
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            <Badge
                              variant="secondary"
                              className={`${categoryInfo.textColor} bg-white dark:bg-gray-800`}
                            >
                              {categoryInfo.name}
                            </Badge>
              ...
              <Card className="border-dashed">
                <CardContent className="p-8 text-center">
                  ...
                  <p className="text-sm text-gray-400 mt-1">
                    Add your first trophy to get started!
                  </p>
                </CardContent>
              </Card>
            )}
          ...
          ([categoryKey, categoryInfo]) => (
            ...
            >
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-gray-600 dark:text-gray-400">
                      {stats[categoryKey]}{" "}
                      {stats[categoryKey] === 1 ? "trophy" : "trophies"} earned
                    </p>
                  </div>
                </div>
              </div>
              ...
              <ScrollArea className="h-[600px] p-1">
                ...
                {currentTrophies.filter(
                  (trophy) => trophy.category === categoryKey
                ).length === 0 && (
                  <Card className="border-dashed">
                    <CardContent className="p-8 text-center">
                      {getTrophyIcon(
                        categoryKey,
                        "h-12 w-12 mx-auto mb-3 text-gray-400"
                      )}
                      <p className="text-gray-500 dark:text-gray-400">
                        No {categoryInfo.name.toLowerCase()} yet
                      </p>
                      <p className="text-sm text-gray-400 mt-1">
                        Add your first{" "}
                        {categoryInfo.name.toLowerCase().slice(0, -1)} to get
                        started!
                      </p>
                    </CardContent>
                  </Card>
