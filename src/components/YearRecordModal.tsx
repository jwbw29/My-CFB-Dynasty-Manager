...
const YearRecordModal: React.FC<YearRecordModalProps> = ({ year, onClose }) => {
  ...
  const [heismanWinner, setHeismanWinner] = useState<HeismanWinner>({
    name: "",
    position: "",
    class: "",
    school: "",
  ...
  const formatHeismanString = (heisman: HeismanWinner): string => {
    ...
    return [heisman.name, heisman.position, heisman.class, heisman.school]
      .filter(Boolean)
      .join(" - ");
  };
    ...
    const loadData = () => {
      try {
        ...
        const populatedRecord = {
          ...existingRecord,
          playerAwards: existingRecord.playerAwards?.length
            ? existingRecord.playerAwards
            : getYearAwards(year),
          recruits: existingRecord.recruits?.length
            ? existingRecord.recruits
            : getRecruits(year),
          transfers: existingRecord.transfers?.length
            ? existingRecord.transfers
            : getTransfers(year),
  ...
  const removeDraftedPlayer = (index: number) => {
    if (!record) return;
    const updatedDraft = (record.playersDrafted || []).filter(
      (_, i) => i !== index
    );
    handleFieldChange("playersDrafted", updatedDraft);
  ...
  const DevTraitBadge: React.FC<DevTraitBadgeProps> = ({ trait }) => {
    const colors = {
      Elite:
        "bg-green-500 text-green-900 dark:bg-green-500 dark:text-green-900",
    ...
    return (
      <span
        className={`inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium ${colors[trait]}`}
      >
        {trait}
      </span>
    );
  ...
  const scheduleContent = (
    <div className="space-y-4 p-4 max-w-6xl mx-auto">
      <div className="text-center">
        <h2 className="text-2xl font-bold">{year} Schedule & Results</h2>
      </div>
          ...
          return (
            ...
            >
              ...
              <div className="col-span-1 text-center">
                <Select
                  value={game.location || "vs"}
                  onValueChange={(value: Game["location"]) =>
                    updateSchedule(index, "location", value)
                  }
                >
                  ...
                  <SelectContent>
                    ...
                    <SelectItem value="neutral">N</SelectItem>
                  </SelectContent>
                </Select>
              </div>
              <div className="col-span-5 flex items-center gap-2">
                ...
                <Input
                  value={game.opponent}
                  onChange={(e) =>
                    updateSchedule(index, "opponent", e.target.value)
                  }
              ...
              <div className="col-span-2">
                <Input
                  value={game.score}
                  onChange={(e) =>
                    updateSchedule(index, "score", e.target.value)
                  }
                  placeholder="Score"
              ...
              <div className="col-span-3">
                <Select
                  value={game.result}
                  onValueChange={(value: Game["result"]) =>
                    updateSchedule(index, "result", value)
                  }
                >
                ...
                <div className="space-y-4 p-4 max-w-6xl mx-auto">
                  <div className="text-center space-y-2">
                    <div className="flex items-center justify-center gap-4">
                      {(() => {
                        ...
                        const teamData = coachProfile?.schoolName
                          ? getTeamWithLogo(coachProfile.schoolName)
                          : null;
                        return teamData ? (
                          <>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                    ...
                    <Card>
                      <CardContent className="pt-4">
                        <div className="flex items-center gap-3">
                          <Medal className="h-8 w-8 text-red-500" />
                          <div>
                            <p className="text-xs font-medium text-gray-500">
                              Points Against
                            </p>
                  ...
                  <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
                    ...
                    <Card>
                      <CardHeader className="p-4">
                        <CardTitle className="text-lg flex items-center gap-2">
                          <TrendingUp /> Statistical Summary
                        </CardTitle>
                      </CardHeader>
                      <CardContent className="p-4 space-y-3">
                        {(() => {
                          ...
                          const pointsAgainst =
                            parseInt(record.pointsAgainst) || 0;
                          const [wins, losses] = (record.overallRecord || "0-0")
                            .split("-")
                            .map((n) => parseInt(n) || 0);
                              ...
                              <div className="space-y-2">
                                <div className="flex justify-between items-center p-2 bg-green-50 dark:bg-green-900/20 rounded-lg">
                                  ...
                                  <span className="font-bold text-sm">
                                    {totalGames > 0
                                      ? (pointsFor / totalGames).toFixed(1)
                                      : "0.0"}
                                  </span>
                                </div>
                                <div className="flex justify-between items-center p-2 bg-red-50 dark:bg-red-900/20 rounded-lg">
                                  ...
                                  <span className="font-bold text-sm">
                                    {totalGames > 0
                                      ? (pointsAgainst / totalGames).toFixed(1)
                                      : "0.0"}
                                  </span>
                                </div>
                              </div>
                  ...
                  {record.recruits && record.recruits.length > 0 && (
                    <Card>
                      <CardHeader className="p-4">
                        <CardTitle className="text-lg flex items-center gap-2">
                          <Star /> Recruiting Highlights{" "}
                          {record.recruitingClassPlacement && (
                            <span className="ml-auto text-sm font-normal text-gray-500">
                              #{record.recruitingClassPlacement} Nationally
                            </span>
                          )}
                        </CardTitle>
                      </CardHeader>
                      <CardContent className="p-4">
                        <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
                          {record.recruits.slice(0, 6).map((recruit, index) => (
                            <div
                              key={index}
                        ...
                        {record.recruits.length > 6 && (
                          <p className="text-center text-xs text-gray-500 mt-2">
                            +{record.recruits.length - 6} more recruits
                          </p>
                        )}
                      </CardContent>
                    </Card>
                  ...
                  <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                    <div>
                      ...
                      <h3 className="text-xl font-bold flex items-center gap-2 mb-2">
                        <Star className="text-yellow-500" /> Recruiting Class (
                        {(record.recruits || []).length})
                      </h3>
                      <div className="space-y-2">
                        {(record.recruits || []).length > 0 ? (
                          (record.recruits || [])
                            .sort(
                            ...
                            .map((recruit, index) => (
                              ...
                              >
                                ...
                                <DevTraitBadge
                                  trait={recruit.potential as any}
                                />
                              </div>
                            ))
                        ) : (
                    ...
                    <div>
                      ...
                      <h3 className="text-xl font-bold flex items-center gap-2 mb-2">
                        <Users className="text-blue-500" /> Transfer Portal (
                      ...
                      <div className="space-y-2">
                        {(record.transfers || []).length > 0 ? (
                          (record.transfers || [])
                            .sort((a, b) => {
                              if (
                                a.transferDirection === "From" &&
                                b.transferDirection === "To"
                              )
                                return -1;
                              if (
                                a.transferDirection === "To" &&
                                b.transferDirection === "From"
                              )
                                return 1;
                              return 0;
                            })
                      ...
                      <CardContent className="p-3 grid grid-cols-2 gap-3">
                        ...
                        <div>
                          ...
                          <Input
                            ...
                            onChange={(e) =>
                              handleFieldChange("pointsFor", e.target.value)
                            }
                            placeholder="450"
                            className="h-8 mt-1"
                          />
                        ...
                        <div>
                          ...
                          <Input
                            ...
                            onChange={(e) =>
                              handleFieldChange("pointsAgainst", e.target.value)
                            }
                            placeholder="275"
                            className="h-8 mt-1"
                          />
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        ...
                        <div>
                          ...
                          <div className="grid grid-cols-2 gap-2 mt-1">
                            ...
                            <Select
                              value={heismanWinner.class}
                              onValueChange={(v) =>
                                handleHeismanChange("class", v)
                              }
                            >
                              <SelectTrigger className="h-9">
                  ...
                  <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
                    <div>
                      <h3 className="text-lg font-bold flex items-center gap-2 mb-2">
                        <Award className="text-yellow-500" /> Player Awards (
                        {record.playerAwards?.length || 0})
                      </h3>
                      <div className="space-y-2 max-h-48 overflow-y-auto pr-2">
                        {record.playerAwards?.length > 0 ? (
                          ...
                          <p className="text-sm text-center py-4 text-gray-500">
                            No player awards. (Imported automatically)
                          </p>
                        )}
                      </div>
                    </div>
                    <div>
                      <div className="flex justify-between items-center mb-2">
                        <h3 className="text-lg font-bold flex items-center gap-2">
                          <Medal className="text-blue-500" /> NFL Draft (
                          {record.playersDrafted?.length || 0})
                        </h3>
                        <Button onClick={addDraftedPlayer} size="sm">
                          Add Player
                        </Button>
                      </div>
                      <div className="space-y-2 max-h-48 overflow-y-auto pr-2">
                        {record.playersDrafted?.length > 0 ? (
                          record.playersDrafted
                            ...
                            .map((player, index) => (
                              <div
                                key={player.id || index}
                                className="grid grid-cols-[1fr,auto,auto] gap-2 items-center p-2 border rounded-md"
                              >
                                <Input
                                ...
                                <div className="flex items-center gap-1">
                                  <span className="text-xs">Rnd</span>
                                  <Input
                                    type="number"
                                    value={player.round}
                                    onChange={(e) =>
