...
const calculateQBR = (stat: Partial<PlayerStat>): number => {
  ...
  const rating =
    (8.4 * yards + 330 * touchdowns + 100 * completions - 200 * interceptions) /
    attempts;
...
const PlayerCard: React.FC<PlayerCardProps> = ({ player, isOpen, onClose }) => {
  ...
  useEffect(() => {
    ...
    try {
      ...
      if (playerRecruit) {
        foundRecruitingInfo = {
          year: playerRecruit.recruitedYear,
          stars: playerRecruit.stars,
          type: "Recruited",
        };
      } else {
        ...
        if (playerTransfer) {
          foundRecruitingInfo = {
            year: playerTransfer.transferYear,
            stars: playerTransfer.stars,
            type: "Transfer",
          };
        }
      }
              ...
              <div className="flex-1 min-w-0">
                ...
                <div className="grid grid-cols-2 md:grid-cols-5 gap-3 text-sm">
                  <div>
                    ...
                    <div className="font-semibold">{enhancedPlayer.year}</div>
                  </div>
                  <div>
                    <div className="text-gray-400 text-xs">RATING</div>
                    <div className="font-semibold">{enhancedPlayer.rating}</div>
                  </div>
                  {recruitingInfo && (
                    <>
                      <div>
                        <div className="text-gray-400 text-xs">
                          {recruitingInfo.type.toUpperCase()}
                        </div>
                      ...
                      <div>
                        <div className="text-gray-400 text-xs">
                          {recruitingInfo.type.toUpperCase()} STARS
                        </div>
                        <div className="font-semibold">
                          {"⭐".repeat(parseInt(recruitingInfo.stars))}
                        </div>
                      </div>
                    </>
                  )}
                  {playerAwards.length > 0 && (
                    <div>
                      ...
                      <div className="font-semibold">{playerAwards.length}</div>
                    </div>
                  )}
                </div>
              </div>
            ...
            <div className="bg-white text-black flex-1 flex flex-col overflow-hidden">
              ...
              >
                <TabsList className="w-full rounded-none border-b bg-gray-50 h-10 flex-shrink-0">
                  <TabsTrigger
                    value="stats"
                    className="flex-1 text-black data-[state=active]:bg-white data-[state=active]:text-blue-700 data-[state=active]:border-b-2"
                            ...
                            return Array.from(categories).map((category) => {
                              ...
                              const columns: {
                              ...
                              }[] =
                                {
                                  ...
                                  "Kick Return": [
                                    { key: "krattempts", label: "KR ATT" },
                                    { key: "kryards", label: "KR YRD" },
                                    { key: "krtd", label: "KR TD" },
                                    { key: "krlong", label: "KR LNG" },
                                  ],
                                  "Punt Return": [
                                    { key: "prattempts", label: "PR ATT" },
                                    { key: "pryards", label: "PR YRD" },
                                    { key: "prtd", label: "PR TD" },
                                    { key: "prlong", label: "PR LNG" },
                                  ],
                                }[category as string] || [];
                              const getStatValue = (
                                ...
                                switch (key) {
                                  case "avg":
                                    if (category === "Passing") {
                                      const yards = isFullStat
                                        ? (stat as PlayerStat).passyards
                                        : (stat as { [key: string]: number })
                                            .passyards;
                                      const attempts = isFullStat
                                        ? (stat as PlayerStat).attempts
                                        : (stat as { [key: string]: number })
                                            .attempts;
                                      return (attempts || 0) > 0
                                        ? (
                                    ...
                                    if (category === "Rushing") {
                                      const yards = isFullStat
                                        ? (stat as PlayerStat).rushyards
                                        : (stat as { [key: string]: number })
                                            .rushyards;
                                      const carries = isFullStat
                                        ? (stat as PlayerStat).carries
                                        : (stat as { [key: string]: number })
                                            .carries;
                                      return (carries || 0) > 0
                                        ? (
                                    ...
                                    if (category === "Receiving") {
                                      const yards = isFullStat
                                        ? (stat as PlayerStat).recyards
                                        : (stat as { [key: string]: number })
                                            .recyards;
                                      const receptions = isFullStat
                                        ? (stat as PlayerStat).receptions
                                        : (stat as { [key: string]: number })
                                            .receptions;
                                      return (receptions || 0) > 0
                                        ? (
                                            (yards || 0) / (receptions || 1)
                                          ).toFixed(1)
                                        : "0.0";
                                    }
                                    if (category === "Punting") {
                                      const yards = isFullStat
                                        ? (stat as PlayerStat).puntyards
                                        : (stat as { [key: string]: number })
                                            .puntyards;
                                      const punts = isFullStat
                                        ? (stat as PlayerStat).punts
                                        : (stat as { [key: string]: number })
                                            .punts;
                                      return (punts || 0) > 0
                                  ...
                                  case "fgpct":
                                    const made = isFullStat
                                      ? (stat as PlayerStat).fgmade
                                      : (stat as { [key: string]: number })
                                          .fgmade;
                                    const attempted = isFullStat
                                      ? (stat as PlayerStat).fgattempted
                                      : (stat as { [key: string]: number })
                                          .fgattempted;
                                  ...
                                  case "tackles":
                                    const solo = isFullStat
                                      ...
                                      : (stat as { [key: string]: number })
                                          .solo;
                                    const assists = isFullStat
                                      ? (stat as PlayerStat).assists
                                      : (stat as { [key: string]: number })
                                          .assists;
                                    return (
                                      (solo || 0) + (assists || 0)
                                    ...
                                    <table className="w-full">
                                      <thead>
                                        <tr className="border-b border-gray-200">
                                          <th className="text-left px-4 py-2 text-xs font-medium text-gray-600 uppercase tracking-wide w-16">
                                            YEAR
                                          </th>
                                          {columns.map((col) => (
                                            <th
                                              key={col.key}
                                              className={`text-center px-2 py-2 text-xs font-medium text-gray-600 uppercase tracking-wide ${
                                                col.width || "w-16"
                                              }`}
                                            >
                                              {col.label}
                                      ...
                                      <tbody>
                                        {categoryStats.map((stat, index) => (
                                          ...
                                          >
                                            ...
                                            {columns.map((col) => (
                                              <td
                                                key={col.key}
                                                className="px-2 py-3 text-sm text-gray-900 text-center"
                                              >
                                                {getStatValue(stat, col.key)}
                                              </td>
                                            ))}
                                          </tr>
                                        ))}
                                        <tr className="bg-blue-50 border-t-2 border-blue-200 font-semibold">
                                          ...
                                          {columns.map((col) => (
                                            <td
                                              key={col.key}
                                              className="px-2 py-3 text-sm text-gray-900 text-center font-semibold"
                                            >
                                              {getStatValue(
                                                careerTotals,
                                                col.key,
