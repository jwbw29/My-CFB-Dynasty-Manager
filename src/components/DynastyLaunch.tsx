...
const DynastyLaunch: React.FC<DynastyLaunchProps> = ({ onDynastySelected }) => {
      ...
      try {
        ...
        parsed.data.forEach((row: any) => {
          if (row.Team && row["Color 1"] && row["Color 2"]) {
            presets[row.Team] = {
              primary: row["Color 1"],
              secondary: row["Color 2"],
              accent: row["Color 3"] || row["Color 1"],
            };
          }
        });
  ...
  useEffect(() => {
    ...
    const handleVisibilityChange = () => {
      if (!document.hidden) loadDynasties();
    };
  ...
  const calculateDynastyStatsFromImport = (
    data: ImportedData
  ): { wins: number; losses: number; seasonsPlayed: number } => {
  ...
  const createDynasty = useCallback(() => {
    ...
    if (!newDynasty.schoolName) {
      toast.error("School required");
      return;
    }

    const actualSchoolName =
      newDynasty.schoolName === "CUSTOM_TEAM"
        ? newDynasty.customTeamName.trim()
        : newDynasty.schoolName;
    if (!actualSchoolName) {
    ...
    const newDynastyEntry: Dynasty = {
      ...
      currentYear: newDynasty.currentYear,
  ...
  const importDynasty = useCallback(async () => {
    ...
    try {
      ...
      localStorage.setItem(
        `dynasty_${newDynastyEntry.id}`,
        JSON.stringify(snapshot.dynastyData)
      );
  ...
  const calculateDynastyRecord = (dynastyId: string) => {
    try {
      ...
      if (data.yearRecords && Array.isArray(data.yearRecords)) {
        ...
        data.yearRecords.forEach((record: any) => {
          if (record.overallRecord && record.year) {
            const [wins, losses] = record.overallRecord
              .split("-")
      ...
      if (currentYear && !completedSeasons.has(currentYear)) {
        ...
        if (
          data[currentScheduleKey] &&
          Array.isArray(data[currentScheduleKey])
        ) {
          const schedule = data[currentScheduleKey];
          const currentYearWins = schedule.filter(
            (game: any) => game.result === "Win"
          ).length;
          const currentYearLosses = schedule.filter(
            (game: any) => game.result === "Loss"
          ).length;
      ...
      console.error(
        `Error calculating record for dynasty ${dynastyId}:`,
        error
      );
      return { wins: 0, losses: 0, seasonsPlayed: 0 };
    }
      ...
      <div className="w-full max-w-6xl">
        ...
        <div className="text-center mb-8 flex justify-center gap-4">
          ...
          <Button
            ...
            size="lg"
            variant="outline"
            className="border-2 border-white bg-transparent text-white hover:bg-white hover:text-blue-900 px-8 py-4 text-lg"
          >
                  ...
                  <Card key={dynasty.id} className="bg-white/10 ...">
                    ...
                    <CardContent className="space-y-3 pt-3">
                      <div className="flex items-center gap-2 text-blue-100">
                        ...
                        <span>Year: {dynasty.currentYear}</span>
                      </div>
                      <div className="flex items-center gap-2 text-blue-100">
                        <User className="h-4 w-4" />
                        <span>Seasons: {actualRecord.seasonsPlayed}</span>
                      </div>
                      <div className="text-blue-100">
                        <span>
                          Record: {actualRecord.wins}-{actualRecord.losses}
                        </span>
                      </div>
                      <div className="text-blue-100">
                        <span>
                          ...
                          <AlertDialogContent>
                            <AlertDialogHeader>
                              ...
                              <AlertDialogDescription>
                                ...
                                {dynasty.schoolName} dynasty? This action cannot
                                be undone.
                              </AlertDialogDescription>
                            </AlertDialogHeader>
                            <AlertDialogFooter>
                              ...
                              <AlertDialogAction
                                onClick={() => deleteDynasty(dynasty.id)}
                              >
                                Delete
                              </AlertDialogAction>
                            </AlertDialogFooter>
          ...
          <Card className="bg-white/10 backdrop-blur-sm border-white/20 text-center py-12">
            <CardContent>
              ...
              <p className="text-blue-200 mb-6">
                Create your first dynasty or import an existing one to begin.
              </p>
            </CardContent>
          </Card>
        )}
        <Dialog open={showCreateModal} onOpenChange={setShowCreateModal}>
          <DialogContent className="sm:max-w-[425px]">
            <DialogHeader>
              <DialogTitle>Create New Dynasty</DialogTitle>
            </DialogHeader>
            <div className="grid gap-4 py-4">
              <div className="space-y-2">
                ...
                <Input
                  id="coachName"
                  value={newDynasty.coachName}
                  onChange={(e) =>
                    setNewDynasty({ ...newDynasty, coachName: e.target.value })
                  }
              ...
              <div className="space-y-2">
                ...
                <Select
                  value={newDynasty.schoolName}
                  onValueChange={(value) =>
                    setNewDynasty({ ...newDynasty, schoolName: value })
                  }
                >
                  <SelectTrigger>
                  ...
                  <SelectContent>
                    <SelectItem value="CUSTOM_TEAM">
                      Custom/TeamBuilder Team
                    </SelectItem>
                    {fbsTeams.map((team) => (
                ...
                {newDynasty.schoolName === "CUSTOM_TEAM" && (
                  <div className="space-y-2 mt-2">
                    ...
                    <Input
                      id="customTeamName"
                      value={newDynasty.customTeamName}
                      onChange={(e) =>
                        setNewDynasty({
                          ...newDynasty,
                          customTeamName: e.target.value,
                        })
                      }
            ...
            <DialogFooter>
              ...
              <Button onClick={createDynasty}>Create Dynasty</Button>
            </DialogFooter>
          </DialogContent>
        </Dialog>
        <Dialog open={showImportModal} onOpenChange={setShowImportModal}>
          <DialogContent className="sm:max-w-[500px]">
            <DialogHeader>
              <DialogTitle>Import Dynasty</DialogTitle>
            </DialogHeader>
            <div className="grid gap-4 py-4">
              <div className="space-y-2">
            ...
            <DialogFooter>
              ...
              <Button
                onClick={importDynasty}
                disabled={!selectedFile || isImporting}
              >
                {isImporting ? "Importing..." : "Import Dynasty"}
              </Button>
            </DialogFooter>
          </DialogContent>
        </Dialog>
