...
type StatKeys = keyof Omit<
  PlayerStat,
  "id" | "playerName" | "year" | "category"
>;
type StatCategory =
  | "Passing"
  | "Rushing"
  ...
  switch (category) {
    ...
    case "Rushing":
      return players.filter((player) =>
        ["QB", "RB", "FB", "WR"].includes(player.position),
      );
    case "Receiving":
      return players.filter((player) =>
        ["WR", "TE", "RB", "FB"].includes(player.position),
      );
    case "Blocking":
      return players.filter((player) =>
    ...
    case "Kick Return":
      return players.filter((player) =>
        ["RB", "WR", "CB", "FS", "SS"].includes(player.position),
      );
    case "Punt Return":
      return players.filter((player) =>
        ["RB", "WR", "CB", "FS", "SS"].includes(player.position),
...
const getDefaultSortConfig = (
  ...
  switch (category) {
    ...
    case "Punting":
      return { key: "Punt Yards", direction: "desc" };
    case "Kick Return":
...
const PlayerStats: React.FC = () => {
  ...
  const [playerStats, setPlayerStats] = useLocalStorage<PlayerStat[]>(
    "playerStats",
    [],
  );
  ...
  const [newStat, setNewStat] =
    useState<Omit<PlayerStat, "id">>(initialStatState);
  ...
  const [sortConfig, setSortConfig] = useState<{
    key: string;
    direction: "asc" | "desc";
  }>(() => getDefaultSortConfig(selectedCategory));
    ...
    (playerName: string) => {
      ...
      if (player) {
        const playerForCard = {
          ...
          devTrait:
            (player.devTrait as "Normal" | "Impact" | "Star" | "Elite") ||
            "Normal",
          notes: player.notes || "",
          isRedshirted: false,
        };
    ...
    if (editingId) {
      setPlayerStats(
        playerStats.map((stat) =>
          stat.id === editingId
            ? ({ ...newStat, id: editingId } as PlayerStat)
            : stat,
        ),
      );
    ...
    filteredStats.forEach((stat) => {
      ...
      if (!playerTotals[playerKey]) {
        playerTotals[playerKey] = {
          ...stat,
          year: 0,
          id: `career-${playerKey}-${selectedCategory}`,
        };
      } else {
        Object.entries(stat).forEach(([key, value]) => {
          if (typeof value === "number" && key !== "year" && key !== "id") {
            ...
            if (key.toLowerCase().includes("long")) {
              (playerTotals[playerKey] as any)[key] = Math.max(
                currentTotal,
                value,
              );
            } else {
  ...
  const getStatValue = useCallback(
    (stat: PlayerStat, category: string): string | number => {
      ...
      if (category === "FG %")
        return `${calculateCompletionPercentage(stat.fgmade, stat.fgattempted)}%`;
      if (category === "XP %")
        return `${calculateCompletionPercentage(stat.xpmade, stat.xpattempted)}%`;
      if (category === "Tackles")
        return Number(stat.solo || 0) + Number(stat.assists || 0);
      if (category === "YPP")
        return calculateAverage(stat.puntyards, stat.punts);
  ...
  const sortedStats = useMemo(() => {
    ...
    return viewType === "Career"
      ? sorted.slice(0, careerStatsDisplayCount)
      : sorted;
  }, [
    getDisplayStats,
    sortConfig,
    viewType,
    careerStatsDisplayCount,
    getStatValue,
  ]);

  const totalCareerStats = useMemo(
    () => (viewType === "Career" ? getDisplayStats.length : 0),
    [viewType, getDisplayStats],
  );
  const hasMoreCareerStats =
    viewType === "Career" && careerStatsDisplayCount < totalCareerStats;
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-center gap-4">
        ...
        <div className="flex items-center gap-2">
          <Select
            value={viewType}
            onValueChange={(value: "Season" | "Career") => setViewType(value)}
          >
          ...
          <Select
            ...
            onValueChange={(value: StatCategory) => setSelectedCategory(value)}
          >
      ...
      <Dialog open={isModalOpen} onOpenChange={setIsModalOpen}>
        <DialogContent className="sm:max-w-4xl">
          <DialogHeader>
            <DialogTitle>
              {editingId ? "Edit Player Stats" : "Add New Player Stats"}
            </DialogTitle>
          </DialogHeader>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 py-4">
            <div className="flex flex-col space-y-1.5 lg:col-span-4">
              ...
              <Select
                ...
                onValueChange={(v) =>
                  setNewStat((p) => ({ ...p, playerName: v }))
                }
                disabled={!!editingId}
              >
                <SelectTrigger id="playerName">
      ...
      <Card>
        <CardHeader>
          <CardTitle>
            {viewType} Stats - {selectedCategory}
          </CardTitle>
        </CardHeader>
        <CardContent>
          <Table>
                  ...
                  <TableHead
                    ...
                    onClick={() =>
                      setSortConfig((prev) => ({
                        key: stat,
                        direction:
                          prev.key === stat && prev.direction === "asc"
                            ? "desc"
                            : "asc",
                      }))
                    }
            ...
            <TableBody>
              {sortedStats.map((stat) => (
                <TableRow key={stat.id}>
                  <TableCell>
                    <button
                      onClick={() => handlePlayerNameClick(stat.playerName)}
          ...
          {(hasMoreCareerStats || canShowLess) && (
            <div className="flex justify-center gap-2 mt-4">
              {hasMoreCareerStats && (
                <Button
                  onClick={() =>
                    setCareerStatsDisplayCount((p) => p + ITEMS_PER_PAGE)
                  }
                  variant="outline"
              ...
              {canShowLess && (
                <Button
                  onClick={() =>
                    setCareerStatsDisplayCount((p) =>
                      Math.max(5, p - ITEMS_PER_PAGE),
                    )
                  }
                  variant="outline"
                  className="flex items-center gap-2"
